<!DOCTYPE html>
<html>
<head>
	<title></title>
	<meta charset="utf-8">
	<link rel="stylesheet" type="text/css" href="/OWeb/css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="/OWeb/css/stylefront.css">
	<link href="https://fonts.googleapis.com/css?family=Lora|Merriweather" rel="stylesheet">
	<script type="text/javascript" src="/OWeb/js/jquery-3.1.1.min.js"></script>
	<script type="text/javascript" src="/OWeb/js/fonctions.js"></script>
</head>

<body>
	
	<nav class="navbar navbar-inverse">
	  <div class="container-fluid">
	    <!-- Brand and toggle get grouped for better mobile display -->


	    <div class="navbar-header">
	      <a class="" href="/OWeb">
	        <img alt="OLibrary" src="/OWeb/img/potter-books.png" width="80px" height="80px" id="navbar-logo">
	      </a>
	    </div>

	    <!-- Collect the nav links, forms, and other content for toggling -->
	    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
	      <ul class="nav navbar-nav">
	        <li class="margin-navbar">
	        	<a href="/OWeb" class="library_name">
	        		<span >Au Libre Harry</span>
	        		<span class="sr-only">(current)</span>
	        		<!-- <img src="/OWeb/img/hedwige.png" alt="Potter" width="60px" height="60px"> -->
	        	</a>
	        </li>
	      </ul>
	      
	      <ul class="nav navbar-nav navbar-right">
	        <li class="margin-navbar"><a href="/OWeb/connexion/" class="potter-font">
	       	 	<img src="/OWeb/img/nimbus.png" alt="Potter" width="30px" height="30px">
	        	<span>Connexion</span>
	        </a></li>
	        <li class="margin-navbar"><a href="/OWeb/inscription/" class="potter-font">
	        	<img src="/OWeb/img/choipeau.png" alt="Potter" width="30px" height="30px">
	        	<span>Inscription</span>
	        </a></li>
	      </ul>	
	    </div><!-- /.navbar-collapse -->
	  </div><!-- /.container-fluid -->
	</nav>

</body>

</html>